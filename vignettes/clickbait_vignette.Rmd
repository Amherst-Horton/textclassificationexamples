...
Using `mutate()`, we can apply each of these functions to the titles in our data sets, and as such create our features. Note that in the case of `positivity`, we have to use `rowwise()` to ensure that we evaluate the positivity of each title individually instead of together. 
...
Our logistic regression model has an accuracy of around 0.806, which is a clear improvement over the null model. 

### Decision Tree 

```{r decision-fit, warning = FALSE, out.width='75%'}
# MODEL FIT ------------------------------------------------------------------------------
